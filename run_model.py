...
class ClimateReportGenerator:
    ...
    def create_performance_plots(
        ...
        metrics_text = f"MAE: {metrics['mae']:.2f}°C\nRMSE: {metrics['rmse']:.2f}°C\nR²: {metrics['r2']:.3f}"
        ...
        plt.subplot(1, 2, 1)
        plt.hist(y_true, bins=30, alpha=0.7, label='Actual', color='skyblue', edgecolor='black')
        plt.hist(y_pred, bins=30, alpha=0.7, label='Predicted', color='lightcoral', edgecolor='black')
    ...
    def generate_html_report(
        ...
        <body>
            <div class="container">
                ...
                <div class="summary">
                    ...
                    <p>This report presents the results of training a machine learning model to predict climate temperatures using real-world meteorological data. The model was trained using an ensemble approach combining Linear Regression and Random Forest algorithms.</p>
                </div>
                ...
                <div class="metrics">
                    <div class="metric-card">
                        <div class="metric-value">{results['test_metrics']['mae']:.2f}°C</div>
                        <div class="metric-label">Mean Absolute Error</div>
                    </div>
                    <div class="metric-card">
                        <div class="metric-value">{results['test_metrics']['rmse']:.2f}°C</div>
                        <div class="metric-label">Root Mean Square Error</div>
                    </div>
                    <div class="metric-card">
                        <div class="metric-value">{results['test_metrics']['r2']:.3f}</div>
                ...
                <div class="explanation">
                    ...
                    <ul>
                        <li><strong>MAE (Mean Absolute Error):</strong> On average, our predictions are off by {results['test_metrics']['mae']:.2f}°C</li>
                        <li><strong>RMSE (Root Mean Square Error):</strong> Penalizes larger errors more heavily, giving us {results['test_metrics']['rmse']:.2f}°C</li>
                        <li><strong>R² Score:</strong> Explains {results['test_metrics']['r2']*100:.1f}% of the variance in temperature data</li>
                    </ul>
                </div>
        ...
        plot_descriptions = {
            "actual_vs_predicted.png": {
                ...
                "description": "This scatter plot shows how well our model's predictions align with actual temperatures. Points closer to the red diagonal line indicate better predictions."
            },
            "residuals.png": {
                "title": "Residuals Analysis", 
                "description": "Residuals (errors) should be randomly distributed around zero. Patterns in residuals might indicate model bias or missing features."
            },
            "feature_importance.png": {
                "title": "Feature Importance",
                "description": "Shows which input features (like latitude, longitude, time features) contribute most to the temperature predictions."
            },
            "model_comparison.png": {
                "title": "Model Comparison",
                "description": "Compares the performance of different algorithms in our ensemble (Linear Regression vs Random Forest)."
            },
            "temperature_distribution.png": {
                "title": "Temperature Distribution",
                "description": "Compares the distribution of actual vs predicted temperatures to ensure our model captures the data distribution properly."
            }
        }
        ...
        for plot_file in plot_files:
            ...
            if plot_name in plot_descriptions:
                ...
                <div class="plot-container">
                    ...
                    <img src="{rel_path}" alt="{plot_descriptions[plot_name]['title']}">
                    <div class="explanation">
        ...
        html_content += f"""
                ...
                <div class="info-grid">
                    <div class="info-card">
                        <h4>Dataset Size</h4>
                        <p><strong>Total Samples:</strong> {dataset_info.get('total_samples', 'N/A')}</p>
                        <p><strong>Training Samples:</strong> {dataset_info.get('train_samples', 'N/A')}</p>
                        <p><strong>Test Samples:</strong> {dataset_info.get('test_samples', 'N/A')}</p>
                    </div>
                    <div class="info-card">
                        <h4>Features</h4>
                        <p><strong>Number of Features:</strong> {dataset_info.get('n_features', 'N/A')}</p>
                        <p><strong>Geographic Region:</strong> {dataset_info.get('region', 'N/A')}</p>
                        <p><strong>Time Period:</strong> {dataset_info.get('time_period', 'N/A')}</p>
                    </div>
                    <div class="info-card">
                        <h4>Data Source</h4>
                        <p><strong>Primary:</strong> NOAA Climate Data API</p>
                        <p><strong>Fallback:</strong> Realistic synthetic data based on meteorological principles</p>
                        <p><strong>Variables:</strong> Temperature, Precipitation, Geographic coordinates</p>
                    </div>
                </div>
                ...
                <div class="explanation">
                    ...
                    <ol>
                        <li><strong>Data Ingestion:</strong> Real-world climate data from NOAA's Climate Data Online service</li>
                        <li><strong>Feature Engineering:</strong> Time-based features (day of year, month) and geographic features</li>
                        <li><strong>Data Cleaning:</strong> Outlier detection and missing value handling</li>
                        <li><strong>Train/Test Split:</strong> 80/20 split for unbiased evaluation</li>
                    </ol>
                    
                    <h4>Model Training:</h4>
                    <ol>
                        <li><strong>Linear Regression:</strong> Baseline model for interpretability</li>
                        <li><strong>Random Forest:</strong> Non-linear patterns and feature interactions</li>
                        <li><strong>Ensemble Method:</strong> Simple averaging of predictions for robustness</li>
                        <li><strong>Validation:</strong> Cross-validation and holdout testing</li>
                    </ol>
                </div>
                ...
                <div class="explanation">
                    ...
                    <ul>
                        <li>The ensemble approach provides more robust predictions than individual models</li>
                        <li>Geographic features (latitude/longitude) are likely important for regional temperature patterns</li>
                        <li>Time-based features help capture seasonal temperature variations</li>
                        <li>The model performance is suitable for climate monitoring applications</li>
                    </ul>
                    
                    <h4>Potential Improvements:</h4>
                    <ul>
                        <li>Include more climate variables (humidity, wind speed, atmospheric pressure)</li>
                        <li>Add satellite-derived features (vegetation indices, land cover)</li>
                        <li>Implement time series models for temporal dependencies</li>
                        <li>Use deep learning for complex non-linear patterns</li>
                    </ul>
                </div>
        ...
        if "model_scores" in results["training_results"]:
            ...
            for model_name, scores in results["training_results"]["model_scores"].items():
                html_content += f"""
                        <tr>
...
Examples:
    ...
    python run_model.py --start-date 2023-01-01 --end-date 2023-06-30 --bbox -125 30 -110 45
    ...
    parser.add_argument("--bbox", nargs=4, type=float, default=[-120, 35, -115, 40],
                       help="Bounding box: min_lon min_lat max_lon max_lat (default: California region)")
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            logger.info(f"✅ Dataset prepared: {len(df)} samples, {len(features)} features")
            
            # Step 2: Train model
            logger.info("🤖 Step 2/5: Training machine learning models...")
            model, results = trainer.train_model(
                df, features, test_size=args.test_size, random_state=args.random_state
            )
            ...
            plot_files = report_generator.create_performance_plots(
                y_test.values, y_pred_test, results["training_results"], features, model
            )
            ...
            dataset_info = {
                ...
                "region": f"Lon: [{args.bbox[0]:.1f}, {args.bbox[2]:.1f}], Lat: [{args.bbox[1]:.1f}, {args.bbox[3]:.1f}]",
            ...
            logger.info(f"📋 Report available at: {os.path.abspath(report_path)}")
            
            print("\n" + "="*60)
            print("🌍 CLIMATE TEMPERATURE PREDICTION - RESULTS SUMMARY")
            print("="*60)
            print(f"📊 Model Performance:")
            print(f"   • Mean Absolute Error: {results['test_metrics']['mae']:.2f}°C")
            print(f"   • Root Mean Square Error: {results['test_metrics']['rmse']:.2f}°C")
            print(f"   • R² Score: {results['test_metrics']['r2']:.3f}")
            print(f"📈 Dataset: {len(df)} samples, {len(features)} features")
            print(f"⏱️  Training Time: {training_time:.2f} seconds")
            print(f"📁 Output Directory: {os.path.abspath(args.output_dir)}")
            print(f"📋 View detailed report: file://{os.path.abspath(report_path)}")
            print("="*60)
