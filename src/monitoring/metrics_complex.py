...
class SimpleModelMonitor:
    ...
    def __init__(self):
        ...
        self.model_accuracy_gauge = Gauge(
            "model_accuracy", "Current model accuracy", registry=self.registry
        )
    ...
    def log_batch_prediction(
        self, batch_size: int, total_latency: float
    ) -> None:  # noqa: E501
        ...
        logger.debug(
            f"Logged batch prediction: {batch_size} predictions, {avg_latency:.3f}s avg latency"  # noqa: E501
        )
    ...
    def calculate_model_performance(
        ...
        window = (
            ...
            if window_hours
            else self.monitoring_window  # noqa: E501
        )
        ...
        if not recent_predictions:
            logger.warning(
                "No recent predictions with actual values for performance calculation"  # noqa: E501
            )
            return {}

        # Extract predictions and actual values
        predicted_values = [
            p["prediction"]["predicted_temperature"]
            for p in recent_predictions  # noqa: E501
        ]
        actual_values = [p["actual_value"] for p in recent_predictions]

        # Calculate metrics
        mae = np.mean(
            np.abs(np.array(predicted_values) - np.array(actual_values))
        )  # noqa: E501
        rmse = np.sqrt(
            np.mean(
                (np.array(predicted_values) - np.array(actual_values)) ** 2
            )  # noqa: E501
        )
        mape = (
            np.mean(
        ...
        correlation = (
            np.corrcoef(predicted_values, actual_values)[0, 1]
        ...
        logger.info(
            f"Calculated performance metrics: MAE={mae:.3f}, RMSE={rmse:.3f}, Correlation={correlation:.3f}"  # noqa: E501
        )
    ...
    def detect_data_drift(
        ...
        # Overall drift score
        overall_drift = (
            np.mean(list(drift_scores.values())) if drift_scores else 0
        )  # noqa: E501
        ...
        logger.info(
            f"Data drift detection completed: overall score={overall_drift:.3f}"  # noqa: E501
        )
    ...
    def get_monitoring_dashboard_data(self) -> Dict[str, Any]:
        ...
        prediction_stats = {
            ...
            "avg_latency": (
                np.mean([p["latency"] for p in recent_predictions])
                if recent_predictions
                else 0
            ),
            "error_rate": len(
                [p for p in recent_predictions if p.get("error")]
            )  # noqa: E501
        ...
        return {
            ...
            "monitoring_window_hours": self.monitoring_window.total_seconds()
            / 3600,  # noqa: E501
    ...
    def _check_latency_alert(self, latency: float) -> None:
        ...
        if latency > threshold:
            self.generate_alert(
                "high_latency",
                f"Prediction latency {latency:.3f}s exceeds threshold {threshold}s",  # noqa: E501
                "warning",
    ...
    def _check_performance_alerts(self, metrics: Dict[str, float]) -> None:
        ...
        if metrics.get("mae", 0) > mae_threshold:
            self.generate_alert(
                ...
                f"Model MAE {metrics['mae']:.3f} exceeds threshold {mae_threshold}",  # noqa: E501
                "critical",
    ...
    def _check_drift_alerts(self, drift_score: float) -> None:
        ...
        if drift_score > threshold:
            self.generate_alert(
                "data_drift_detected",
                f"Data drift score {drift_score:.3f} exceeds threshold {threshold}",  # noqa: E501
                "warning",
    ...
    def _cleanup_old_records(self) -> None:
        ...
        cutoff_time = (
            datetime.now() - self.monitoring_window * 2
        )  # Keep 2x window for analysis

        # Clean predictions log
        self.predictions_log = [
            p for p in self.predictions_log if p["timestamp"] >= cutoff_time
        ]
        ...
        self.performance_metrics = [
            ...
            if m["timestamp"] >= cutoff_time  # noqa: E501
        ]

        # Keep alerts for longer (7 days)
        alert_cutoff = datetime.now() - timedelta(days=7)
        self.alerts_log = [
            a for a in self.alerts_log if a["timestamp"] >= alert_cutoff
        ]  # noqa: E501
...
class HealthChecker:
    ...
    def check_model_health(model) -> Dict[str, Any]:
        ...
        health = {
            ...
            "model_fitted": (
                getattr(model, "is_fitted", False) if model else False
            ),  # noqa: E501
            "timestamp": datetime.now(),
        }
    ...
    def check_data_health(data_dir: str) -> Dict[str, Any]:
        ...
        if health["raw_data_available"]:
            ...
            health["latest_raw_file"] = (
                max(raw_files, key=lambda p: p.stat().st_mtime).name
                if raw_files
                else None
            )
...
def main() -> None:
    ...
    for i in range(100):
        ...
        actual_value = prediction["predicted_temperature"] + random.uniform(
            -2, 2
        )  # noqa: E501
    ...
    print(
        f"Dashboard data: {json.dumps(dashboard_data, default=str, indent=2)}"
    )  # noqa: E501
