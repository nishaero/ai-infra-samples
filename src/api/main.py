...
# Prometheus metrics
PREDICTION_COUNTER = Counter('climate_predictions_total', 'Total number of predictions made')
PREDICTION_LATENCY = Histogram('climate_prediction_duration_seconds', 'Time spent on predictions')
ERROR_COUNTER = Counter('climate_prediction_errors_total', 'Total number of prediction errors', ['error_type'])

# Global variables
app = FastAPI(
    title="Climate Temperature Prediction API",
    description="REST API for predicting climate temperature using NASA Earth data",
    version="1.0.0",
    docs_url="/docs",
    redoc_url="/redoc"
)
...
class ModelManager:
    ...
    def load_model(self, model_path: Optional[str] = None) -> ClimatePredictor:
...
def get_model() -> ClimatePredictor:
...
async def get_model_info(current_model: ClimatePredictor = Depends(get_model)):
...
async def predict_temperature(
    ...
    current_model: ClimatePredictor = Depends(get_model)
):
    ...
    try:
        # Create features
        features_df = create_features_from_request(request, model_manager.feature_columns)
...
async def predict_temperature_batch(
    ...
    try:
        ...
        for pred_request in request.predictions:
            # Create features
            features_df = create_features_from_request(pred_request, model_manager.feature_columns)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Climate Prediction API Server")
