...
class SimpleClimatePreprocessor:
    ...
    def load_raw_data(
    ...
    ) -> Dict[str, pd.DataFrame]:  # noqa: E501
        """Load raw data files.
        ...
        for temp_file in temp_files:
            if temp_file.exists():
                data["temperature"] = pd.read_csv(
                    temp_file, parse_dates=["date"]
                )  # noqa: E501
                logger.info(
                    f"Loaded temperature data from {temp_file.name}: {len(data['temperature'])} records"  # noqa: E501
                )
                break
    ...
    def prepare_training_data(
        self, start_date: str, end_date: str, target_col: str = "temperature"
        ...
        # Scale features
        processed_df[feature_cols] = self.scaler.fit_transform(
            processed_df[feature_cols]
        )

        # Save processed data
        output_file = (
            self.processed_dir / f"training_data_{start_date}_{end_date}.csv"
        )  # noqa: E501
        processed_df.to_csv(output_file, index=False)
        logger.info(f"Saved processed training data to {output_file}")

        logger.info(
            f"Created training dataset: {len(processed_df)} samples, {len(feature_cols)} features"  # noqa: E501
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--target", default="temperature", help="Target column"
    )  # noqa: E501
    ...
    df, features = preprocessor.prepare_training_data(
        args.start_date, args.end_date, args.target
    )
